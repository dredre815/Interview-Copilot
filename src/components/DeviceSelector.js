        ...
        <div className="device-sections">
          ...
          <div className="device-section">
            ...
            <div className="device-note">
              ...
              <div>
                <p>When selecting System Audio:</p>
                <ul>
                  <li>Check "Share system audio" in the dialog</li>
            ...
            <div className="device-list">
              {devices.speakers.map(device => (
                <button
                  key={device.id}
                  className={`device-option ${selectedDevices.speaker?.id === device.id ? 'selected' : ''}`}
                  onClick={() => handleDeviceSelect({...device, type: 'speaker'})}
                >
                  ...
                  {selectedDevices.speaker?.id === device.id && (
                    <span className="device-selected material-icons">check_circle</span>
                  )}
                </button>
              ))}
            </div>
          ...
          <div className="device-section">
            ...
            <div className="device-list">
              {devices.microphones.map(device => (
                <button
                  key={device.id}
                  className={`device-option ${selectedDevices.microphone?.id === device.id ? 'selected' : ''}`}
                  onClick={() => handleDeviceSelect({...device, type: 'microphone'})}
                >
